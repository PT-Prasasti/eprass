...
function newform(row_id, product_inquiry, rand_id, obj){
    ...
    if (IS_READONLY) {
        var html = `
            <div class="forwarder-information forwarder-`+ rand_id +` forwarder-`+row_id+`">
                <div class="row m-0">
                    <div class="col-12">
                        ...
                        <div>
                            ...
                            <p class="mb-1">`+obj.forwarder_name+`</p>
                        </div>
                        <div class="">
                            <small>Price</small>
                        ...
                        <div>
                            <small>Track</small>
                            <p class="mb-1">`+obj.track+`</p>
                        </div>
                        <div>
                            <small>Description</small>
                            <p class="mb-1">`+obj.description.toUpperCase()+`</p>
                        </div>                        
                    </div>
                </div>
            </div>
        ...
        $(FORWARDER_OPT).each(function(k,v){
            if (obj) {
                if (obj.forwarder_id == v.id) {
                    forwarder_options = forwarder_options + `<option value="`+v.id+`" selected>`+v.forwarder_name+`</option>`;
                } else {
                    forwarder_options = forwarder_options + `<option value="`+v.id+`">`+v.forwarder_name+`</option>`;
                }
            } else {
                forwarder_options = forwarder_options + `<option value="`+v.id+`">`+v.forwarder_name+`</option>`;
                obj = {};
            ...
            <div class="forwarder-information forwarder-`+ rand_id +` forwarder-`+row_id+`">
                <div class="row m-0">
                    <div class="col-12">
                        ...
                        <div>
                            <small>Forwarder Name</small>
                            <select name="forwarder_id[`+product_inquiry+`][]" class="form-control forwarder-form" data-formid="`+rand_id+`" onchange="forwarder_change(`+row_id+`, $(this))">
                        ...
                        <div class="">
                            <small>Price</small>
                            <input type="text" placeholder="Price" name="price[`+product_inquiry+`][]" class="form-control le-price" onchange="form_change(`+row_id+`,'product_price', $(this))" data-formid="`+rand_id+`" value="`+ (obj.product_price ?? '') +`">
                        </div>
                        <div>
                        <small>Track</small>
                        <select name="track[`+product_inquiry+`][]" class="form-control" required>
                        ...
                        <div>
                            <small>Description</small>
                            <textarea name="description[`+product_inquiry+`][]" cols="30" rows="4" placeholder="Product Description" class="form-control" onchange="form_change(`+row_id+`,'product_desc', $(this))" data-formid="`+rand_id+`">`+ (obj.product_desc ?? '') +`</textarea>
                        </div>
                        <div class="remove-action" data-rowid="`+row_id+`" data-randid="`+rand_id+`">
