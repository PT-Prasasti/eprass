...
function init_readonly() {
    $('.carl-long-row').each(function () {
        ...
        rowid = $(this).attr('data-rowid');
        product_inquiry = $(this).attr('data-prodinq');
        status_so = $('input[name="status_so"]').val();
        // form = localget("form" + so_id);
        // console.log("init", form);
...
function newform(row_id, product_inquiry, rand_id, obj) {
    ...
    if (IS_READONLY) {
        var html =
            `
            <div class="supliyer-information supliyer-` +
            rand_id +
            ` supliyer-` +
            row_id +
            `">
            ...
            rand_id +
            `">
                            <input type="radio" name="product_` +
            product_inquiry +
            `" value="` +
            obj.id +
            `" style="zoom:1.5" ` +
            (status_so === 'true' ? 'checked="checked"' : '') +
            `>
                        </div>
                        <div>
        ...
        $(SUPLIYER_OPT).each(function (k, v) {
            if (obj) {
                ...
                supliyer_options =
                    supliyer_options + `<option value="` + v.id + `">` + v.company + `</option>`;
                obj = {};
            }
        ...
        var html =
            ...
            product_inquiry +
            `][]" class="form-control supliyer-form" data-formid="` +
            rand_id +
            `" onchange="supliyer_change(` +
            row_id +
            `, $(this))">
            ...
            `</textarea>
                        ...
                        <div>
                            <small>Qty</small>
                            <input type="number" placeholder="Qty" name="product_qty[` +
            product_inquiry +
            `][]" class="form-control" onchange="form_change(` +
            row_id +
            `,'product_qty', $(this))" data-formid="` +
            rand_id +
            `" value="` +
            (obj.product_qty ?? '') +
            `">
                        ...
                        <div>
                            <small>Currency</small>
                            <select name="product_curentcy[` +
            product_inquiry +
            `][]" class="form-control" onchange="form_change(` +
            row_id +
            `,'product_curentcy', $(this))" data-formid="` +
            rand_id +
            `">
                                <option value="">-Select Curency-</option>
                                <option value="idr" ` +
            ((obj.product_curentcy ?? '') == 'idr' ? 'selected' : '') +
            `>IDR</option>
                                <option value="usd" ` +
            ((obj.product_curentcy ?? '') == 'usd' ? 'selected' : '') +
            `>USD</option>
                            </select>
                        </div>
                        <div>
            ...
            `">
                        ...
                        <div>
                            <small>Production Time</small>
                            <p>
                                <input type="text" placeholder="Production Time" name="production_time[` +
            product_inquiry +
            `][]" class="form-control" onchange="form_change(` +
            row_id +
            `,'production_time', $(this))" data-formid="` +
            rand_id +
            `" value="` +
            (obj.production_time ?? '') +
            `">
